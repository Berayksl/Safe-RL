...
hidden_dim = 256
rewards     = []
model_path = '../models/sac_v2'

target_region_center = [0.0, 3]
target_region_radius = 0.5
action_range = 4 #max speed and max turning rate
        ...
        for eps in range(max_episodes):
            ...
            for step in range(max_steps):
                if frame_idx < explore_steps:
                    ...
                    action_RL = sac_trainer.policy_net.get_action(state, deterministic = DETERMINISTIC)
                    if cbf:
                ...
                #print(action)
                if cbf:
                    next_state, reward, done = env.step(action, TCBF.gamma(step, target_region_radius))
                else:
