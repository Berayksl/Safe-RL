...
from tcbf_sac.utils import soft_update, hard_update, to_tensor
from tcbf_sac.model import GaussianPolicy, QNetwork, DeterministicPolicy
from tcbf_sac.diff_tcbf_qp import CBFQPLayer
import numpy as np


class TCBF_SAC(object):
    ...
    def __init__(self, num_inputs, action_space, env, args):
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_space.shape[0], args.hidden_size, env).to(self.device)
        ...
        if self.cbf_mode != 'off':
            self.cbf_layer = CBFQPLayer(env)


    def select_action(self, state, time = None, evaluate=False, warmup=False, safe_action=True, cbf_info=None):
        ...
        if warmup:
            ...
            for i in range(batch_size):
                action[i] = torch.from_numpy(self.sample_action()).to(self.device)
        else:
        ...
        if safe_action:
            final_action = self.get_safe_action(state, action, time, cbf_info=cbf_info)
    ...
    def update_parameters(self, memory, batch_size, updates, time, cbf_info, memory_model=None, real_ratio=None):
        """
        ...
        with torch.no_grad():
            ...
            if self.cbf_mode == 'full' or self.cbf_mode == 'mod':
                next_state_action = self.get_safe_action(next_state_batch, next_state_action, next_t, modular=self.cbf_mode == 'mod', cbf_info = cbf_info)
        ...
        if self.cbf_mode == 'full' or self.cbf_mode == 'mod':
            pi = self.get_safe_action(state_batch, pi, next_t, modular=self.cbf_mode == 'mod', cbf_info = cbf_info)
    ...
    def get_safe_action(self, state_batch, action_batch, time, modular=False, cbf_info=None):
        ...
        safe_action_batch = self.cbf_layer.get_safe_action(state_batch, action_batch, t = time, modular=modular, cbf_info=cbf_info)

        return safe_action_batch


    def sample_action(self):
        action_min = self.env.action_min
        action_max = self.env.action_max

        return np.random.uniform(low=action_min, high=action_max)
