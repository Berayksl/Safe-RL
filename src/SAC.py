...
class SAC:
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        #
        self.action_range = hyperparameters.get("action_range", [5, 3]) #max speed and max turning rate
    ...
    # def rollout(self, max_timesteps_per_episode=200):
    #     state =  self.env.reset()
    #     explore_steps = 0 #might need to change later!
    #     episode_reward = 0

    #     for step in range(max_timesteps_per_episode):
    #         if self.frame_idx < explore_steps:
    ...
    # def train(self):
    #     i_so_far = 0 #num of iterations so far
    #     self.frame_idx = 0 #num of frames so far
    ...
    #         #collect experience
    #         self.rollout(self.max_timesteps_per_episode)
    ...
    def train(self):
        ...
        target_region_center = self.CBF_params['target_region_center']
        target_region_radius = self.CBF_params['target_region_radius']
        ...
        for eps in range(self.num_episodes):
            ...
            for step in range(self.max_timesteps_per_episode):
                if frame_idx < explore_steps:
                    ...
                    if self.CBF:
                        ...
                        if not target_reached:
                            action_CBF = TCBF.solve_cbf_qp(TCBF.CBF, TCBF.g_func, TCBF.f_func, state, target_region_center, step, epsilon, action_RL, target_region_radius) #for linear TCBF
                            if action_CBF is None:
                                # If the QP fails, we can either ignore the CBF action or set it to zero
                                action= (0.0, 0.0)
                                print('RL action:', action_RL)
                ...
                if self.CBF:
                    next_state, reward, done = self.env.step(action, TCBF.gamma(step, target_region_radius))
                else:
                    next_state, reward, done = self.env.step(action)
