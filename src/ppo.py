...
class PPO:
	...
	def learn(self, total_timesteps):
		...
		plt.title(f'CBF: {self.CBF}, Violations: {self.safety_violations}, Episodes: {len(self.logger["avg_batch_rews"])}, Updates per Iterations: {self.n_updates_per_iteration}')
		...
		while t < self.timesteps_per_batch:
			...
			for ep_t in range(int(self.max_timesteps_per_episode)):
				...
				if self.CBF:
					...
					action = (action_CBF[0] + action_RL[0] , action_CBF[1] + action_RL[1]) #a_rl + a_cbf

					if abs(action[0]) < 0.0001:
						print('here')
						print(action)
						action = (0.0, action[1])
				else:
